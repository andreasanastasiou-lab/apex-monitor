...
export default function AddDeviceModal({ onClose, onAdded }) {
  const [form, setForm] = useState({
    name: '', ip: '', type: 'workstation', group_name: 'Workstations',
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.monitors.includes('icmp')) { setError('At least ICMP monitoring must be selected.'); return }
    ...
    setLoading(true)
    try {
      await axios.post('/api/devices/inventory', {
        name: form.name.trim(),
        ip: form.ip.trim(),
        type: form.type,
      ...
      setError(detail || 'Failed to add device.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800">
          <h2 className="text-sm font-semibold text-gray-100">Add Device</h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Device Name *">
              <input required value={form.name} onChange={set('name')}
                className={inputCls} placeholder="e.g. DESKTOP-01" />
            </Field>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="Location">
              <input value={form.location} onChange={set('location')}
                className={inputCls} placeholder="e.g. Office A, Rack 3" />
            </Field>
            <Field label="Owner">
